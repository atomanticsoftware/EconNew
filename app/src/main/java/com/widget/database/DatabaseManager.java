...
public enum DatabaseManager {
	 ...
	 */
	public void storeDataStatii(ArrayList<MeListItem> meLists,
		...
		for (MeListItem meListItem : meLists) {
			...
			cv.put(DatabaseHelper.Data_HEADING, meListItem.fullName);
			cv.put(DatabaseHelper.Data_DESCRIPTION, meListItem.description);
			cv.put(DatabaseHelper.WIDGET_ID, Integer.toString(appWidgetId));
			cv.put(DatabaseHelper.USER_ID, meListItem.userId);
			cv.put(DatabaseHelper.Data_IMAGE, meListItem.imageUrl);
			cv.put(DatabaseHelper.Data_DATE, meListItem.DataDate);
			cv.put(DatabaseHelper.Data_ID, meListItem.id);
			db.insert(DatabaseHelper.DataS_TABLE_NAME, null, cv);
		}
		meLists.clear();
	 ...
	 *            widget info,Data status from database
	 */
	public void deleteStoredValueOfWidget(String appWidgetId) {
		db.delete(DatabaseHelper.DataS_TABLE_NAME, DatabaseHelper.WIDGET_ID
	...
	/**
	 * @param appWidgetId
	 * @return Fetch stored Datas in accordance with appWidgetId
	 */
	public ArrayList<MeListItem> fetchStoredDataStatii(String appWidgetId) {
		ArrayList<MeListItem> meList = new ArrayList<MeListItem>();
		Cursor cursor = db.query(DatabaseHelper.DataS_TABLE_NAME, null,
				DatabaseHelper.WIDGET_ID + "=?", new String[] { appWidgetId },
				null, null, null);
	    //Gravity.C
		while (cursor.moveToNext()) {
			...
			meListItem.fullName = cursor.getString(cursor
					.getColumnIndex(DatabaseHelper.Data_HEADING));
			meListItem.description = cursor.getString(cursor
					.getColumnIndex(DatabaseHelper.Data_DESCRIPTION));
			meListItem.DataDate = cursor.getString(cursor
					.getColumnIndex(DatabaseHelper.Data_DATE));
			meListItem.imageUrl = cursor.getString(cursor
					.getColumnIndex(DatabaseHelper.Data_IMAGE));
			meListItem.imagePath = cursor.getString(cursor
					.getColumnIndex(DatabaseHelper.Data_IMAGE_FILENAME));
			meListItem.id = cursor.getString(cursor
					.getColumnIndex(DatabaseHelper.Data_ID));
	...
	public void saveFileName(int appWidgetId, String idValue, String fileName) {
		...
		cv.put(DatabaseHelper.Data_IMAGE_FILENAME, fileName);
		int effectedRows = db.update(DatabaseHelper.DataS_TABLE_NAME, cv,
				DatabaseHelper.WIDGET_ID + "=? and " + DatabaseHelper.Data_ID
	...
	public String getDataListName(int appWidgetId){
		Cursor cursor = db.query(DatabaseHelper.WIDGET_INFO_TABLE_NAME,
